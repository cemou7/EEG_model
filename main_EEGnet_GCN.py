...
os.environ["CUDA_VISIBLE_DEVICES"]="0"          # if use '2,1', then in pytorch, gpu2 has id 
...
def get_model(model_name, device, model_classification):
    if model_name == 'LMDANet':
        ...
        eeg_net = EEGNet_ATTEN(Chans=22,kernLength1=36,kernLength2=24,kernLength3=18,F1=16,D=2,classification_num=4,DOR=0.5)
...
if __name__ == '__main__':
    ...
    code = '13'
    all_test_acc = []
    model_name = 'EEGNet_ATTEN'     # LMDANet   EEG_Conformer   PatchTST    iTransformer    MixFormer   dfformer EEGNet  LMDANet_GCN EEGNet_ATTEN
    data_name = 'data_2a_TE_EEGNet_ATTEN_ResidualMLP'    #'2a' 'data2a_no_reject'  'data_2a_TE_GCN'   
    ...
    # if True:
        ...
        # ============================ 
        train_dataloader, test_loader = get_data_bci2a_TE(subject_id=subject_id, batch_size=batch_size)# 分类数据
        ...
        # exp.run()
        _, _,best_test_acc = exp.run_no_val_EEG()
        all_test_acc.append(best_test_acc)

        # 自监督训练
        # pos_loader, neg_loader,test_loader = get_contrastive_dataloaders(batch_size=batch_size)
    with open(summary_dir, "a") as f:
        ...
        f.write("==================================================\n")
        # exp.run_contrastive_finetune_prescreening(
