...
class PatchEmbed(nn.Module):
    def __init__(
        self,
        layers: List[Tuple[int, int, int, int]],    # [[1, 64, 125, 1], [64, 64, 8, 4], [64, 64, 4, 2]]
        bias: bool = False,
    ):
        super().__init__()
    ...
    def forward(self, x):
        for block in self.blocks:   # (1408, 1, 1000) -> (1408, 64, 876) -> (1408, 64, 218) -> (1408, 64, 108)
            x = block(x)
...
class Embedding(nn.Module):
    def __init__(
        ...
        self.temporal_token = nn.Parameter(torch.zeros(1, 1, self.embed_dim))       # (1, 1, 64)
    ...
    def forward(self, x, register_hook=False):
        ...
        # Path embedding
        x = x.reshape(B * C, D, T)  # BC x D x T    (1408, 1, 1000)
        x = self.patch_embed(x)                   # (1408, 64, 108) == (64 * 22, 64, 108)

        # Temporal position embedding & block
        x = x.transpose(1, 2)  # BC x T x D         (64 * 22, 108, 64)
        ...
        B, T, C, D = x.shape
        x = x.reshape(B * T, C, D)  # BT x C x D    (64 * 109, 22, 64)
...
def set_parser():
    parser = argparse.ArgumentParser(description='DF-former')

    parser.add_argument('--random_seed', type=int, default=2024, help='random seed')
    parser.add_argument('--lr', default=0.003, help='dim')
    parser.add_argument('--dim', type=int, default=64, help='dim')
    parser.add_argument('--nhead', type=int, default=4, help='nhead')
    parser.add_argument('--nlayer', type=int, default=2, help='nlayer')
    parser.add_argument('--batch_size', type=int, default=64, help='batch_size')
    parser.add_argument('--cnn_layers', default=[[1, 64, 125, 1], [64, 64, 8, 4], [64, 64, 4, 2]], help='cnn_layers')

    parser.add_argument('--db_name', default="BCIC2a", help='db_name')
    parser.add_argument('--num_classes', default=4, help='num_classes')
    parser.add_argument('--num_subjects', default=9, help='num_subjects')
    parser.add_argument('--train_subject_num', default=8, help='train_subject_num')
    parser.add_argument('--inter_information_length', default=22, help='inter_information_length')
    parser.add_argument('--origin_ival', default=[0, 1000], help='origin_ival')
    parser.add_argument('--seq_len', default=108, help='seq_len')
    parser.add_argument('--EPOCHS', default=100, help='EPOCHS')
    parser.add_argument('--target_subject', default=0, help='target_subject')
    parser.add_argument('--target_session', default=0, help='target_session')
    parser.add_argument('--is_subject_independent', default=True, help='is_subject_independent')
    parser.add_argument('--apply_cls_head', default=True, help='apply_cls_head')
    parser.add_argument('--use_token', default=False, help='use_token')
    parser.add_argument('--model_name', default="dfformer", help='model_name')

    args = parser.parse_args()
    return args


def get_dfformer_model():
    args = set_parser()
    model = DFformer(args)
    return model
...
if __name__ == "__main__":
    model = get_dfformer_model().cuda()
    a = torch.randn(64, 22, 1000).cuda().float()
    a = a.unsqueeze(2)
    outputs = model(a)
