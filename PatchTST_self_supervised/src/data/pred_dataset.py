...
class Dataset_ETT_hour(Dataset):
    ...
    def __read_data__(self):
        ...
        df_raw = pd.read_csv(os.path.join(self.root_path,
                                          self.data_path))      # DataFrame: (17420, 8)

        border1s = [0, 12 * 30 * 24 - self.seq_len, 12 * 30 * 24 + 4 * 30 * 24 - self.seq_len]  # [0, 8128, 11008]
        border2s = [12 * 30 * 24, 12 * 30 * 24 + 4 * 30 * 24, 12 * 30 * 24 + 8 * 30 * 24]   # [8640, 11520, 14400]
        border1 = border1s[self.set_type]   # 0
        border2 = border2s[self.set_type]   # 8640
        ...
        if self.scale:
            ...
            self.scaler.fit(train_data.values)      # 根据训练集的均值和方差，调整全部数据标准化。
            data = self.scaler.transform(df_data.values)    # [17420, 7]
        else:
            data = df_data.values
        ...
        self.data_stamp = data_stamp    # [8640, 4]

    def __getitem__(self, index):
        s_begin = index     # 0
        s_end = s_begin + self.seq_len  # 512
        r_begin = s_end - self.label_len    # 512
        r_end = r_begin + self.label_len + self.pred_len    # 608

        seq_x = self.data_x[s_begin:s_end]  # [512, 7]
        seq_y = self.data_y[r_begin:r_end]  # [96, 7]
        seq_x_mark = self.data_stamp[s_begin:s_end]     # [512, 4]
        seq_y_mark = self.data_stamp[r_begin:r_end]     # [96, 4]
