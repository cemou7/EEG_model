...
def get_model(model_name, device, model_classification):
    if model_name == 'LMDANet':
        eeg_net = LMDA(num_classes=4, chans=22, samples=1000, classification=model_classification).to(device)
...
def main_train(valid_data_set, data_aug):
    # 随机种子，保证生成的随机数是一样的
    ...
    model_save_path = "model/" + model_name + "_A0.pth"    # _classification  classification_froze    finetone
    classification = True                                      # False: pre-train contrastive net  or  True: fine-tone
    # ============================

    train_dataloader, test_dataloader = get_data_bci2a_cross_validation(batch_size=batch_size, valid_data_set=valid_data_set)
    ...
    best_acc = exp.run_PatchTST_self(data_aug=data_aug)        # False True
    return best_acc


if __name__ == '__main__':

    with open("loop_train.txt", "w") as file:
        for valid_data in ['A01', 'A02', 'A03', 'A04', 'A05', 'A06', 'A07', 'A08', 'A09']:
            file.write(f"Trainning {valid_data}: \n")
            train_acc, best_acc = main_train(valid_data, data_aug=False)
            file.write(f"Train Acc No Aug: {train_acc}\n")
            file.write(f"Best Test Acc No Aug: {best_acc}\n")
            file.flush()

            train_acc, best_acc = main_train(valid_data, data_aug=True)
            file.write(f"Train Acc With Aug: {train_acc}\n")
            file.write(f"Best Test Acc With Aug: {best_acc}\n")
            file.write("\n")
            file.flush()
